...
const mix = (from: Rgb, to: Rgb, amount: number): Rgb => ({ r: Math.round(from.r + (to.r - from.r) * amount), g: Math.round(from.g + (to.g - from.g) * amount), b: Math.round(from.b + (to.b - from.b) * amount) });
const rgb = (value: Rgb) => `rgb(${value.r}, ${value.g}, ${value.b})`;
...
export default function ParticleText({
  ...
  useEffect(() => {
    ...
    if (typeof window === 'undefined' || !container || !canvas) return undefined;
    ...
    const sampleText = async () => {
      ...
      particles = targets.filter((_, index) => index % stride === 0).map((target, index) => {
        ...
        const particleColor = baseRgb && highlightRgb ? rgb(mix(baseRgb, highlightRgb, blend)) : color;
    ...
    const queueSample = () => { if (resizeFrame) window.cancelAnimationFrame(resizeFrame); resizeFrame = window.requestAnimationFrame(() => void sampleText()); };
    ...
    const reduceChange = (event: MediaQueryListEvent) => { reducedMotion = event.matches; void sampleText(); };
