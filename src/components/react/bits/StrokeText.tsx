...
interface StrokeTextBox { x: number; y: number; width: number; height: number }
...
export default function StrokeText({
  ...
  useEffect(() => {
    ...
    const build = () => {
      setStart();
      const timeline = gsap.timeline({ paused: true, repeat: trigger === 'loop' ? -1 : 0, repeatDelay: trigger === 'loop' ? 0.9 : 0, defaults: { overwrite: 'auto' } });
    ...
    if (trigger === 'hover') {
      ...
      timeline = build();
      if (trigger === 'scroll') scrollTrigger = ScrollTrigger.create({ trigger: root, start: 'top 82%', once: true, onEnter: () => timeline?.play(0) });
      else timeline.play(0);
    }
    return () => { removeHover?.(); scrollTrigger?.kill(); timeline?.kill(); gsap.killTweensOf(targets); };
  ...
  return (
    <span ref={rootRef} className={`stroke-text ${trigger === 'hover' ? 'stroke-text--hover' : ''} ${className}`.trim()} style={{ ...style, ['--stroke-text-height' as string]: `${Math.round(fontSize * 1.3)}px` } as CSSProperties} role="img" aria-label={String(text ?? '')}>
