...
### 3.3 `WorkThemeTransition`

职责：在所有非 Work 页面与 Work 之间承载点聚焦门户、Carbon 轨道、百分比和正反主题切换。

- 组件全局挂载于 `Base.astro`，不能只在 Work 条件渲染。
- 每次进入完整显示 `0% → 100%`，每次离开完整显示 `100% → 0%`。
- 百分比表达 Work 主题视口占据进度，不表达网络加载。
- 摄像机路径、门户半径、Hero 比例和轨道共用一个主进度。
- reduced-motion 下直接切换最终主题，不阻塞内容。
- 激活时正确管理 `inert`、焦点和滚动锁，完成、取消或失败后完全清理。
- 完整视觉、路由和架构规则见 [`theme-transition.md`](./theme-transition.md)。
